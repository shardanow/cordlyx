...
7. [Связка с миграциями схемы](#7-связка-с-миграциями-схемы)
...
## 7. Связка с миграциями схемы

Перед восстановлением старого дампа на новый код (и наоборот) сверьтесь с
версиями: дамп содержит данные, но не историю `schema_migrations`.

```bash
./scripts/migrate.sh --list    # что применено / что висит в PENDING
./scripts/migrate.sh           # доставить недостающее (идемпотентно)
```

Порядок при восстановлении: развернуть дамп → `migrate.sh` → `verify-restore.sh`.
Подробности: `RUNBOOK.md` §5, `ARCHITECTURE.md` §12 (DB migrations).
